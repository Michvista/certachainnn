          ...
          <section className="flex-1">
            ...
            <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-gray-500">Manage academic records for Solana University on-chain.</p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <StatCard label="Total Certificates Issued" value="12,842" trend="+12%" sub="48 issued today" />
              <StatCard label="Pending Verifications" value="156" alert="Needs Action" sub="Avg. time: 4.2 hours" />
              <StatCard label="Active Students" value="5,901" trend="Stable" sub="All identities verified" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                ...
                <div className="flex items-end justify-between h-48 gap-2">
                  {[40, 60, 50, 80, 100, 30, 45].map((h, i) => (
                    <div key={i} className="flex flex-col items-center flex-1 gap-4">
                      <div
                        style={{ height: `${h}%` }}
                        className={`w-full rounded-t-sm transition-all ${i === 4 ? 'bg-[#7030d8]' : 'bg-indigo-100'}`}
                      />
                      <span className="text-[10px] font-bold text-gray-400">{['MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT', 'SUN'][i]}</span>
                    </div>
                  ))}
                </div>
              </div>

              <div className="bg-[#111827] p-8 rounded-xl text-white space-y-6 relative overflow-hidden">
                <Search className="text-gray-400" />
                <div>
                  <h3 className="text-xl font-bold">Student Registry</h3>
                  <p className="text-xs text-gray-400 leading-relaxed mt-2">
                    Instantly locate academic records across the Solana Ledger using wallet addresses or student IDs.
                  </p>
                </div>
